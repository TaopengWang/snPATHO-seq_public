...
# arguments ----
data_obj_file_path <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/data/snPATHO_seq/25k_downsampled_processed_objs/integrated/4411_cellbender_filtered_integration_final_annotation.rds'

out_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/integration/25k_downsampled/cellbender/4411/final_plots'
...
# only plot the cancer epithelial cells
subset_obj <- subset(obj, subset = major_annotation == 'Epithelial_cancer')

pdf(file = paste0(out_dir, '/', 'umap_cancer_only.pdf'),
    width = 8)
    print(
        DimPlot(subset_obj, reduction = "umap", group.by = 'major_annotation', cols = cell_type_colors) +
            labs(title = NULL) +
...
plot_order <- plot_mtx_long %>% 
    arrange(factor(cell_type, levels = c(
        'RBCs', 'Epithelial_cancer', 'Cholangiocyte', 'Hepatocyte', 'CAF',
        'Mixed_lymphocytes', 'Macrophage', 'LSEC', 'Endothelial'))) %>% 
    pull(annotation) %>% 
    unique()
...
genes_to_plot <- c(
    'EPCAM', 'ESR1', 'KRT8', 'KRT18', 'GATA3',
    'ONECUT1', 'KRT7', 'KRT19', 'FGFR2', 'ANXA4',
    'COL1A1', 'PDGFRA', 'PDGFRB',
    'PECAM1', 'VWF', 'FLT1',
    'CLEC4G', 'CLEC4M',
    'ITGAX', 'CD68', 'CD14', 'FCGR3A', 'CD163', 'SPP1',
    'ALB', 'APOA1', 'ASGR1', 'RBP4', 
    'TRAC', 'CD3D', 'MS4A1', 'JCHAIN', 'CD8A', 'IGHM', 'IL7R',
    'SPTA1', 'HBB', 'HBA1'
)
...
ggsave(file = paste0(out_dir, '/', 'cell_type_proportion_barplot.pdf'), plot, width = 5, height = 12)
