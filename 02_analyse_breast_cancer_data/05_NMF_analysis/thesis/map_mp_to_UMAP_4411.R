...
out_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/gene_module_analysis/thesis_analysis/map_GM_4411'
...
gene_list[['Stress']] <- gavish_mp[['Stress']]
gene_list[['Stress_in_vitro']] <- gavish_mp[['Stress (in vitro)']]
...
# # only plot on the cancer cells 
# pdf(file = paste0(umap_dir, '/', gs, '_cancer_only.pdf'), width = 8)
#         print(
#             FeaturePlot(
#                 subset(obj, subset = major_annotation == 'Epithelial_cancer'), 
#                 features = gs, cols = c('lightgrey', 'red'),
#                 order = TRUE) +
#                 labs(title = gs)+
