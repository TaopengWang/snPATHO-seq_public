...
# arguments ----
de_res_file <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/gene_module_analysis/4066_unique_GMs/DE_between_GM_cells.rds'
...
lapply(seq_along(reactome_res_list), function(x){
    ...
    plot <- ggplot(working_results, aes(reorder(pathway, NES), NES)) +
        ...
        theme_bw()
    ggsave(file = paste0(out_dir, '/', 'GSEA_', names(reactome_res_list)[[x]], '_only_enriched.pdf'), width = 10, height = 7, plot, device = 'pdf')
})
...
working_module <- 'nmf_4066'
working_pathway <- 'REACTOME_CA2_PATHWAY'
...
working_module <- 'nmf_4066'
working_pathway <- 'REACTOME_INOSITOL_PHOSPHATE_METABOLISM'
