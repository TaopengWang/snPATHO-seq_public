...
# aim ----
    # use seurat module score method to score robust NMF and other programs on single cell data 
...
out_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/gene_module_analysis/map_GM_4399'
...
NMF_4399_unique <- c('ADAMTS10','ANKRD10','ANTXR1','BGN','CADPS2','CDH3','CEMIP2','COL5A1','CSF1R','DZIP1L','FAM189A2','GABRE','IL18BP','INPP5B','LDLRAD4','MCF2L','MFAP2','P3H2','PALM2-AKAP2','PDZRN3','PHLDB1','PMEPA1','PRKCD','RND1','RNF121','TNC','TNS3','VWA1','WNK2','ZNF521')

NMF_list <- list()
for (t in c('NMF_cell_cycle', 'NMF_hypoxia', 'NMF_stress', 'NMF_4399_unique')) {
...
########################################################################################################################
########################################################################################################################
# map all gene sets on umap
obj <- readRDS(file = obj_file)



working_out <- paste0(out_dir, '/', 'Seurat_module_scores')
