...
# arguments ----
data_file <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/data/external_data/10X_data_integrated/EndocervicalAdenocarcinoma_6707_integrated.rds'

out_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/snRNA_external_data/10X_data_Tony/integration/EndocervicalAdenocarcinoma_6707'
dir.create(out_dir, recursive = T)

sample_id <- 'EndocervicalAdenocarcinoma_6707'
...
new_annotation_df <- data.frame(cluster_id = unique(as.character(Idents(obj))),
                                initial_annotation = '')
new_annotation_df[new_annotation_df$cluster_id == '0', 'initial_annotation'] <- 'CAF'
...
new_annotation_df[new_annotation_df$cluster_id == '6', 'initial_annotation'] <- 'CAF'
new_annotation_df[new_annotation_df$cluster_id == '7', 'initial_annotation'] <- 'Endothelial'
new_annotation_df[new_annotation_df$cluster_id == '8', 'initial_annotation'] <- 'Macrophages'
new_annotation_df[new_annotation_df$cluster_id == '9', 'initial_annotation'] <- 'Monocytes'
new_annotation_df[new_annotation_df$cluster_id == '10', 'initial_annotation'] <- 'SMCs'
new_annotation_df[new_annotation_df$cluster_id == '11', 'initial_annotation'] <- 'B'
new_annotation_df[new_annotation_df$cluster_id == '12', 'initial_annotation'] <- 'Cycling'
new_annotation_df[new_annotation_df$cluster_id == '13', 'initial_annotation'] <- 'Lymphatic_endothelial'
new_annotation_df[new_annotation_df$cluster_id == '14', 'initial_annotation'] <- 'pDC'
...
plot_order <- plot_mtx_long %>% 
    arrange(factor(cell_type, 
        levels = c(
            'Monocytes', 'Macrophages', 'Plasmablasts', 'Endothelial', 'Epithelial', 'Lymphatic_endothelial',
            'CAF', 'SMCs', 'Cycling', 'T_CD4', 'T_CD8', 'B', 'unknown', 'pDC'))) %>% 
    pull(annotation) %>% 
    unique()
