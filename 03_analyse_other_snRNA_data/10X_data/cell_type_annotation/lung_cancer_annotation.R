...
# arguments ----
data_file <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/data/external_data/10X_data_integrated/lungcancer_20011329LC_integrated.rds'

out_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/snRNA_external_data/10X_data_Tony/integration/lungcancer_20011329LC'
dir.create(out_dir, recursive = T)

sample_id <- 'lungcancer_20011329LC'
...
markers <- c(
    ...
    'PCNA', 'RRM2', 'FEN1', 'GINS2', 'TYMS',
    'UCHL1', 'TFF3', 'APOA1BP', 'CLDN3', 'SEC11C', 'NGFRAP1', 'SCG5', 'HIGD1A', 'PHGR1', 'CD24',
    'PCSK1N', 'NR5A1', 'DIO2', 'VGF', 'CLU', 'PDK4', 'SERPINA6', 'PLA2G1B', 'ITM2C')
...
new_annotation_df <- data.frame(cluster_id = unique(as.character(Idents(obj))),
                                initial_annotation = '')
new_annotation_df[new_annotation_df$cluster_id == '0', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '1', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '2', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '3', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '4', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '5', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '6', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '7', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '8', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '9', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '10', 'initial_annotation'] <- 'Endothelial'
new_annotation_df[new_annotation_df$cluster_id == '11', 'initial_annotation'] <- 'Neuroendocrine_cells'
new_annotation_df[new_annotation_df$cluster_id == '12', 'initial_annotation'] <- 'Myeloid'
new_annotation_df[new_annotation_df$cluster_id == '13', 'initial_annotation'] <- 'CAF_PVL'
new_annotation_df[new_annotation_df$cluster_id == '14', 'initial_annotation'] <- 'Epithelial_cycling'
...
plot_order <- plot_mtx_long %>% 
    arrange(factor(cell_type, 
        levels = c(
            'Endothelial', 'CAF_PVL', 'T', 'Myeloid', 'Epithelial_cycling', 'Neuroendocrine_cells'))) %>% 
    pull(annotation) %>% 
    unique()
