...
# arguments ----
data_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/data/external_data/CRCL-Data_other/aligned_data'

datasets <- c('endometrium_T')
# 'endometrium_T' 'colon15_T', 'ovary_T'

out_data_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/data/external_data/CRCL-Data_processed'
dir.create(out_data_dir, recursive = T)

out_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/snRNA_external_data/Cyril_data'
...
for (ds in datasets) {
    ...
    obj <- CreateSeuratObject(counts, project = "U_Lyon_data", assay = "RNA")
    obj[["percent.mt"]] <- PercentageFeatureSet(obj, pattern = "^MT-")

    # populate metadata
    obj$dataset_id <- ds
    obj$processing_method <- 'snPATHO'
    obj$sample_id <- ds
