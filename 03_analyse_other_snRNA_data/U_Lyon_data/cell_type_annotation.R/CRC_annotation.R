...
# arguments ----
data_file <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/data/external_data/CRCL-Data_processed/colon15_T_clustered.rds'

out_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/results/snRNA_external_data/Cyril_data/colon15_T'
dir.create(out_dir, recursive = T)

sample_id <- 'colon15_T'

out_data_dir <- '/share/ScratchGeneral/tonwan/projects/snPATHO-Seq/data/external_data/CRCL-Data_annotated'
...
markers <- c(
    ...
    'VCAM1', 'IGHG1', 'XBP1', 'MZB1',
    'CD24', 'CD44', 'EPCAM',
    'S100A8', 'S100A9',
    'VWA5A', 'RGS13', 'C1orf186', 'HPGDS', 'CPA3', 'GATA2', 'MS4A2', 'KIT', 'TPSAB1', 'TPSB2',
    'SOX10', 'MPZ', 'GAP43', 'EGR2',
    'REG4', 'TPH1', 'AFP', 'SLC18A1', 'LMX1A', 'TRPA1', 'FGF1', 'PIEZO2', 'ADGRG4', 'ADM', 'CRH', 'GJB1',
    'AVIL', 'DCLK1', 'POU2F3', 'SIGLECF', 'CDHR2', 'ESPN', 'TSLP')
...
for (cl in c(5, 19, 20, 8, 12, 21, 23, 24)) {
...
new_annotation_df <- data.frame(cluster_id = unique(as.character(Idents(obj))),
                                initial_annotation = '')
new_annotation_df[new_annotation_df$cluster_id == '0', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '1', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '2', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '3', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '4', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '5', 'initial_annotation'] <- 'SMC'
new_annotation_df[new_annotation_df$cluster_id == '6', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '7', 'initial_annotation'] <- 'T'
new_annotation_df[new_annotation_df$cluster_id == '8', 'initial_annotation'] <- 'CAF'
new_annotation_df[new_annotation_df$cluster_id == '9', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '10', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '11', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '12', 'initial_annotation'] <- 'CAF'
new_annotation_df[new_annotation_df$cluster_id == '13', 'initial_annotation'] <- 'Macrophages'
new_annotation_df[new_annotation_df$cluster_id == '14', 'initial_annotation'] <- 'Endothelial'
new_annotation_df[new_annotation_df$cluster_id == '15', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '16', 'initial_annotation'] <- 'Plasmablasts'
new_annotation_df[new_annotation_df$cluster_id == '17', 'initial_annotation'] <- 'Epithelial'
new_annotation_df[new_annotation_df$cluster_id == '18', 'initial_annotation'] <- 'Monocytes'
new_annotation_df[new_annotation_df$cluster_id == '19', 'initial_annotation'] <- 'Pericytes'
new_annotation_df[new_annotation_df$cluster_id == '20', 'initial_annotation'] <- 'Mast_cells'
new_annotation_df[new_annotation_df$cluster_id == '21', 'initial_annotation'] <- 'Schwann_cells'
new_annotation_df[new_annotation_df$cluster_id == '22', 'initial_annotation'] <- 'B'
new_annotation_df[new_annotation_df$cluster_id == '23', 'initial_annotation'] <- 'Mixed'
new_annotation_df[new_annotation_df$cluster_id == '24', 'initial_annotation'] <- 'Tuft_cells'
...
plot_mtx <- as.data.frame(AverageExpression(obj, assays = 'RNA', slot = 'data', features = top_markers, group.by = c('initial_annotation'))[[1]])
...
plot_mtx_long$cell_type <- plot_mtx_long$annotation
...
plot_order <- plot_mtx_long %>% 
    arrange(factor(cell_type, 
        levels = c(
            'Mast_cells', 'T', 'B', 'Mixed', 'Plasmablasts', 'Schwann_cells', 'SMC',
            'Pericytes', 'CAF', 'Macrophages', 'Endothelial', 'Tuft_cells', 'Epithelial', 'Monocytes'))) %>% 
    pull(annotation) %>% 
    unique()
...
annot_df <- plot_mtx_long[, c('annotation', 'cell_type')]
