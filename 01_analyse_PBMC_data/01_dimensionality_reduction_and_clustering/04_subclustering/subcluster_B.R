...
# aim ----
    # subcluster B cells to better differentiate them
...
subset_obj <- subset(obj, subset = manual_annotation %in% c('B_intermediate_memory', 'B_naive'))
...
working_dir <- paste0(out_dir, '/', 'reclustering', '/', 'B_cells')
...
intermediate_B <- c('MS4A1', 'TNFRSF13B', 'IGHM', 'IGHD', 'AIM2', 'CD79A', 'LINC01857', 'RALGPS2', 'BANK1', 'CD79B')
memory_B <- c('MS4A1', 'COCH', 'AIM2', 'BANK1', 'SSPN', 'CD79A', 'TEX9', 'RALGPS2', 'TNFRSF13C', 'LINC01781')
naive_B <- c('IGHM', 'IGHD', 'CD79A', 'IL4R', 'MS4A1', 'CXCR4', 'BTG1', 'TCL1A', 'CD79B', 'YBX3')
plasmablast <- c('IGHA2', 'MZB1', 'TNFRSF17', 'DERL3', 'TXNDC5', 'TNFRSF13B', 'POU2AF1', 'CPNE5', 'HRASLS2', 'NT5DC2')


markers <- c(intermediate_B, memory_B, naive_B, plasmablast, 'TRAC')

DefaultAssay(subset_obj) <- 'RNA'
to_plot <- unique(markers[markers %in% rownames(subset_obj)])
for (g in to_plot) {
...
Idents(subset_obj) <- grep(paste0('res.', 0.2), colnames(subset_obj@meta.data), value = T)[1]
...
new_annotation_df[new_annotation_df$cluster_id == '0', annotation_col_name] <- 'B_naive'
new_annotation_df[new_annotation_df$cluster_id == '1', annotation_col_name] <- 'B_intermediate'
new_annotation_df[new_annotation_df$cluster_id == '2', annotation_col_name] <- 'B_memory'
new_annotation_df[new_annotation_df$cluster_id == '3', annotation_col_name] <- 'doublet'
new_annotation_df[new_annotation_df$cluster_id == '4', annotation_col_name] <- 'B_naive'
...
write.csv(file = paste0(working_dir, '/', 'modified_annotation.csv'), 
    subset_obj@meta.data[, annotation_col_name, drop = F])
