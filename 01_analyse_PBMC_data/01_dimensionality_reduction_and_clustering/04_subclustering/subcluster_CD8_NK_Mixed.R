...
subset_obj <- subset(obj, subset = manual_annotation %in% c('CD8_NK_mixed_CTL_population'))
...
working_dir <- paste0(out_dir, '/', 'reclustering', '/', 'CD8_NK_mixed_CTL_population')
...
markers <- c(NK, CD8_TEM, 'CD8A', 'CD8B', 'CD4', 'NCAM1', 'TRAV10', 'TRAJ18', 'TRAC', 'TRBC1', 'TRAV24')
...
Idents(subset_obj) <- grep(paste0('res.', 0.4), colnames(subset_obj@meta.data), value = T)[1]
...
new_annotation_df[new_annotation_df$cluster_id == '2', annotation_col_name] <- 'NK'
