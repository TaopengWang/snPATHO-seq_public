...
# aim ----
    # some naive CD8 t cells appear to have been mixed with CD8 TCM
...
subset_obj <- subset(obj, subset = manual_annotation %in% c('CD8_TCM_CD8_naive', 'CD8_TCM', 'CD8_naive'))
...
working_dir <- paste0(out_dir, '/', 'reclustering', '/', 'CD8_TCM_CD8_naive')
...
other_markers <- c('CD4', 'PRF1')

markers <- c(CD8_t_cells, naive_CD8, CD8_TCM, other_markers)
...
Idents(subset_obj) <- grep(paste0('res.', 0.8), colnames(subset_obj@meta.data), value = T)[1]
...
new_annotation_df[new_annotation_df$cluster_id == '0', annotation_col_name] <- 'CD8_naive'
new_annotation_df[new_annotation_df$cluster_id == '1', annotation_col_name] <- 'CD8_TCM'
new_annotation_df[new_annotation_df$cluster_id == '2', annotation_col_name] <- 'CD8_TCM'
new_annotation_df[new_annotation_df$cluster_id == '3', annotation_col_name] <- 'CD8_TCM'
new_annotation_df[new_annotation_df$cluster_id == '4', annotation_col_name] <- 'CD8_naive'
new_annotation_df[new_annotation_df$cluster_id == '5', annotation_col_name] <- 'CD4_TEM'
new_annotation_df[new_annotation_df$cluster_id == '6', annotation_col_name] <- 'CD8_naive'
new_annotation_df[new_annotation_df$cluster_id == '7', annotation_col_name] <- 'CD8_naive'
new_annotation_df[new_annotation_df$cluster_id == '8', annotation_col_name] <- 'CD8_naive'
